...
# 라이브 /law/article이 실제로 돌려주는 형태(2026-07-31 실측 복사) — 호가 **항과 같은
# 줄에** 이어 붙는다. 합성 픽스처(호를 줄바꿈으로 분리)로만 검증했다가 실데이터에서
# 탐지기가 통째로 침묵한 사고가 있었다. 이 픽스처가 그 재발을 막는다.
REAL_API_SHAPE = (
    "지방보조금법 제21조\n"
    "① 지방보조사업자는 지방보조금으로 취득하거나 그 효용이 증가된 것으로서 대통령령으로 "
    "정하는 중요한 재산(이하 \"중요재산\"이라 한다)에 대해서는 그 현재액과 증감을 명백히 "
    "하여야 하고, 그 현황을 지방자치단체의 장에게 보고하여야 한다.\n"
    "② 지방자치단체의 장은 제1항에 따라 중요재산의 현황을 보고받은 경우 대통령령으로 "
    "정하는 바에 따라 그 보고받은 현황을 공시하여야 한다. <신설 2023.4.11>\n"
    "③ 지방보조사업자는 해당 지방보조사업을 완료한 후에도 지방자치단체의 장의 승인 없이 "
    "중요재산에 대하여 다음 각 호의 행위를 하여서는 아니 된다. <개정 2023.4.11> "
    "1. 교부 목적 외 용도로의 사용 2. 양도, 교환 또는 대여 3. 담보의 제공\n"
    "④ 지방보조사업자가 다음 각 호의 어느 하나에 해당하는 경우에는 지방자치단체의 장의 "
    "승인을 받지 아니하고도 제3항 각 호의 행위를 할 수 있다. <개정 2023.4.11> "
    "1. 지방보조금의 전부를 반환한 경우 2. 지방자치단체의 장이 정한 기간이 지난 경우\n"
    "⑤ 지방자치단체의 장은 지방보조사업자가 승인 없이 중요재산에 대하여 제2항 각 호의 "
    "행위를 한 경우에는 다음 각 호의 금액의 반환을 명할 수 있다. <개정 2023.4.11> "
    "1. 중요재산을 취득하기 위하여 사용된 지방보조금에 해당하는 금액 "
    "2. 중요재산의 효용가치 증가액에 해당하는 금액\n"
)


def test_detects_anomaly_in_real_api_response_shape():
    """호가 인라인으로 붙는 실제 응답 형태에서도 탐지된다(침묵 사고 회귀)."""
    found = detect_crossref_anomalies(REAL_API_SHAPE)
    assert len(found) == 1, f"실제 형태에서 탐지 실패: {found}"
    assert found[0]["hang"] == 5 and found[0]["referenced"] == 2
    assert 3 in found[0]["candidates"] and 4 in found[0]["candidates"]


def test_inline_amendment_date_is_not_a_ho():
    """'<개정 2023.4.11>'이 인라인으로 있어도 호로 오인하지 않는다."""
    txt = ("제9조\n"
           "① 목적을 정한다. <개정 2023.4.11>\n"
           "② 제1항 각 호에 따른다. <신설 2024.1.5>\n")
